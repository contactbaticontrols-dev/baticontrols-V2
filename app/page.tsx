...
export default function Home() {
  ...
  const { ref: secteursRef, inView: secteursInView } = useInView();
  ...
  return (
    <>
      ...
      {/* SECTEURS — Hôtellerie prioritaire */}
      <section className="py-28 bg-[#F0F4FF]" ref={secteursRef}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className={`text-center mb-16 transition-all duration-700 ${secteursInView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
            <span className="text-[#E8611A] font-semibold text-sm uppercase tracking-widest">Secteurs d'intervention</span>
            <h2 className="text-3xl sm:text-4xl font-black text-[#0D1F5C] mt-3">Nos marchés cibles</h2>
          </div>
          ...
          <div className={`mb-10 transition-all duration-700 delay-100 ${secteursInView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
            <div className="bg-[#0D1F5C] rounded-2xl p-10 flex flex-col md:flex-row items-center gap-8">
              ...
              <div className="flex-1 text-center md:text-left">
                ...
                <p className="text-blue-200 leading-relaxed">Supervision centralisée de toutes les installations techniques, optimisation du confort client, régulation CVC par chambre, gestion énergétique et réduction des coûts d'exploitation.</p>
              </div>
              <div className="flex flex-col gap-3 text-sm text-blue-200 min-w-[200px]">
                {["Régulation CVC par zone", "Éclairage LED dimmable", "Comptage énergie temps réel", "Supervision centralisée GTB"].map(item => (
                  <div key={item} className="flex items-center gap-2">
                    <svg className="w-4 h-4 text-[#E8611A] shrink-0" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" /></svg>
                    {item}
                  </div>
                ))}
              </div>
